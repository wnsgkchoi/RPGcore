    ...
    fun generateEffectLore(action: EffectAction): String {
        ...
        val description = when (type.uppercase()) {
            ...
            "COOLDOWN_RESET" -> {
                ...
                "&7스킬 사용 시 &b${String.format("%.1f", chance)}%&7 확률로 재사용 대기시간이 적용되지 않습니다."
            }
            ...
            "COOLDOWN_REDUCTION_ON_HIT" -> {
                val chance = params["chance"]?.toDoubleOrNull()?.times(100) ?: 0.0
                val reduction = (params["reduction_ticks"]?.toDoubleOrNull() ?: 0.0) / 20.0
                "&7피격 시 &b${String.format("%.1f", chance)}%&7 확률로 모든 스킬의 재사용 대기시간이 &b${String.format("%.1f", reduction)}초&7 감소합니다."
            }
            ...
            "BURST_DAMAGE_NEGATION" -> {
                val threshold = (params["damage_threshold_percent_max_hp"]?.toDoubleOrNull() ?: 0.0) * 100
                val negation = (params["negation_percent_of_excess"]?.toDoubleOrNull() ?: 0.0) * 100
                val cooldown = (params["cooldown_ticks"]?.toDoubleOrNull() ?: 0.0) / 20.0
                "&7최대 체력의 &c${String.format("%.0f", threshold)}%&7를 초과하는 피해를 받으면, &b${String.format("%.1f", cooldown)}초&7에 한 번 초과분의 &a${String.format("%.0f", negation)}%&7를 무시합니다."
            }
            ...
            // Slaughterer Set (그림자 학살자)
            "ATTACK_SPEED_BUFF" -> {
                val duration = (params["duration_ticks"]?.toDoubleOrNull() ?: 0.0) / 20.0
                val boost = (params["attack_speed_bonus"]?.toDoubleOrNull() ?: 0.0) * 100
                "&7치명타 시 &b${String.format("%.1f", duration)}초&7 동안 공격 속도가 &a${String.format("%.0f", boost)}%&7 증가합니다."
            }
            "HIGH_HP_BONUS_DAMAGE" -> {
                val threshold = (params["health_threshold_percent"]?.toDoubleOrNull() ?: 0.0) * 100
            ...
            "DOUBLE_STRIKE" -> {
                ...
                "&7모든 공격 시 &b${String.format("%.1f", chance)}%&7 확률로 피해가 2회 적용됩니다."
            }

            // Set Bonus specific effect descriptions
            "ON_ATTACK_COOLDOWN_REDUCTION" -> {
                val chance = params["chance"]?.toDoubleOrNull()?.times(100) ?: 0.0
                val reduction = (params["reduction_ticks"]?.toDoubleOrNull() ?: 0.0) / 20.0
                "&7공격 시 &b${String.format("%.1f", chance)}%&7 확률로 모든 스킬의 재사용 대기시간이 &b${String.format("%.1f", reduction)}초&7 감소합니다."
            }
            "OUT_OF_COMBAT_SHIELD" -> {
                val shield = (params["shield_percent_max_hp"]?.toDoubleOrNull() ?: 0.0) * 100
                val interval = (params["check_interval_ticks"]?.toLongOrNull() ?: 100L) / 20
                "&7비전투 상태가 &b${interval}초&7 이상 지속되면 최대 체력의 &a${String.format("%.0f", shield)}%&7만큼 보호막을 얻습니다."
            }
            "DAMAGE_REDUCTION_WHILE_SHIELDED" -> {
                val reduction = params["reduction_percent"]?.toDoubleOrNull() ?: 0.0
                "&7보호막이 활성화된 동안 받는 모든 피해가 &a${String.format("%.0f", reduction)}%&7 감소합니다."
            }
            "CRITICAL_BOOST" -> {
                val chance = (params["crit_chance"]?.toDoubleOrNull() ?: 0.0) * 100
                val damage = (params["crit_damage"]?.toDoubleOrNull() ?: 0.0) * 100
                "&7치명타 확률 &a+${String.format("%.1f", chance)}%&7, 치명타 피해량 &a+${String.format("%.0f", damage)}%&7"
            }
