...
## 11. RPGCore+ xp 상점 시스템  
### 기본 아이디어  
- xp를 사용하여 특정 아이템을 구매할 수 있도록 한다.  
### 판매 품목  
1. 기본 품목  
- 모든 기본 품목은 상점 GUI 1번째 줄에 나타남. (즉, 최대 9개로 고정)  
- 귀환주문서  
    - 개인마다 특정 월드의 특정 좌표에 스폰포인트 설정 가능.
    - 스폰포인트 설정은 맨 처음만 무료, 이후 xp를 소모.
    - 귀환주문서 사용 시 스폰포인트로 이동.  
- 기초적인 음식  
    - 단순 농사로 얻기 쉬운 음식  
2. 거래장 품목  
- 플레이어는 경매장에 특정 물품을 올릴 수 있음.  
- 아이템을 선택 후, 원하는 판매가(xp)를 입력한 뒤, 상점에 상품을 등록할 수 있음.
- 이 상품들은 상점 GUI 2번째 줄부터 나타나며, 판매되면 품목이 GUI에서 사라져야 함.  
- 서버 종료 후에도 거래장 품목은 유지되어야 함.   

## 12. RPGCore+ 월드 던전 및 레이드 시스템  
### 기본 아이디어  
...
#### 클리어 조건  
- 던전의 최종 도착점을 찾아 5초 이상 밟고 있으면 클리어.
